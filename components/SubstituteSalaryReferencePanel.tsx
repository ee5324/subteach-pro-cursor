...
const SubstituteSalaryReferencePanel: React.FC<SubstituteSalaryReferencePanelProps> = ({ salaryGrades, className = '' }) => {
  ...
  const modalTitleId = 'substitute-salary-reference-modal-title';
    ...
    <>
      <div className={`min-w-0 ${className}`.trim()}>
        <button
          ...
          className="w-full px-4 py-2.5 flex items-center justify-center gap-2 sm:justify-start text-left rounded-xl border border-indigo-200 bg-white/90 hover:bg-white shadow-sm min-h-[44px] transition-colors"
        >
          <Banknote size={20} className="text-indigo-600 shrink-0" />
          <span className="font-bold text-slate-800 text-sm md:text-base">
            代理／代課教師給付參考（依本系統薪級表試算）
          </span>
          <span className="text-xs text-indigo-600 font-medium shrink-0 sm:ml-auto">開啟</span>
        </button>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center p-0 sm:p-4">
          <button
            type="button"
            className="absolute inset-0 bg-black/50 border-0 cursor-pointer w-full h-full"
            aria-label="關閉視窗"
          ...
          <div
            ...
            className="relative z-[101] flex w-full max-w-4xl max-h-[min(92dvh,100vh)] sm:max-h-[90dvh] flex-col rounded-t-2xl sm:rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
              <h2 id={modalTitleId} className="text-base font-bold text-slate-900 pr-2">
                代理／代課教師給付參考
              </h2>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="shrink-0 rounded-lg p-2 text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                aria-label="關閉"
              >
                <X size={22} />
              </button>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-4 md:px-5 md:py-5 space-y-5 text-sm text-slate-700">
            ...
            <div className="shrink-0 border-t border-slate-200 bg-slate-50 px-4 py-3 flex justify-end gap-2 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))]">
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="min-h-[44px] px-5 py-2 rounded-lg bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 shadow-sm"
              >
                關閉
              </button>
            </div>
